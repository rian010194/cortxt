...
export const profiles: Profile[] = [
  { id: 'coordinator', name: 'Coordinator', model: 'nemotron-3-ultra', provider: 'OpenRouter', usage: 'Planning, classification, synthesis, routing', costTier: 'free', latency: '<2s', status: 'verified', skills: ['credential-manager','plan','telemetry','receptionist-base','hermes-agent','hermes-model-routing'], category: 'Orchestration', costPer1MInput: 0, costPer1MOutput: 0, costCurrency: 'USD' },
  { id: 'researcher', name: 'Researcher', model: 'kimi-k2.6', provider: 'Moonshot', usage: 'In-depth analysis, fact-finding, research', costTier: 'low', latency: '<5m', status: 'verified', skills: ['telemetry','arxiv','llm-wiki','ocr-and-documents','youtube-content'], category: 'Research', costPer1MInput: 0.55, costPer1MOutput: 2.65, costCurrency: 'USD' },
  { id: 'planner', name: 'Planner', model: 'kimi-k2.6', provider: 'Moonshot', usage: 'Detailed planning, task breakdown', costTier: 'low', latency: '<5m', status: 'verified', skills: ['plan','test-driven-development','telemetry'], category: 'Orchestration', costPer1MInput: 0.55, costPer1MOutput: 2.65, costCurrency: 'USD' },
  { id: 'builder', name: 'Builder', model: 'kimi-k2.6', provider: 'Moonshot (via Pi)', usage: 'Bounded file writes, code, config, artifacts', costTier: 'medium', latency: 'async', status: 'verified', skills: ['test-driven-development','systematic-debugging','telemetry'], category: 'Implementation', costPer1MInput: 0.55, costPer1MOutput: 2.65, costCurrency: 'USD' },
  ...
  { id: 'credential-manager', name: 'Credential Manager', model: 'nemotron-3-ultra', provider: 'OpenRouter', usage: 'Centralized secret management, vault integration', costTier: 'free', latency: '<2s', status: 'verified', skills: ['credential-manager','telemetry'], category: 'Orchestration', costPer1MInput: 0, costPer1MOutput: 0, costCurrency: 'USD' },
  ...
  { id: 'writer', name: 'Writer', model: 'kimi-k2.6', provider: 'Moonshot', usage: 'Docs, blog posts, PR descriptions, release notes', costTier: 'low', latency: 'async', status: 'experimental', skills: ['telemetry','writer','humanizer'], category: 'Specialist', costPer1MInput: 0.55, costPer1MOutput: 2.65, costCurrency: 'USD' },
  { id: 'plan-auditor', name: 'Plan Auditor', model: 'nemotron-3-ultra', provider: 'OpenRouter', usage: 'Adversarial review cycle (2-3 iterations, fresh eyes)', costTier: 'free', latency: 'async', status: 'experimental', skills: ['telemetry','plan-auditor'], category: 'Review', costPer1MInput: 0, costPer1MOutput: 0, costCurrency: 'USD' },
  { id: 'behaviour-validator', name: 'Behaviour Validator', model: 'nemotron-3-ultra', provider: 'OpenRouter', usage: 'Runs BVC contracts in prod, logs deviations, alerts', costTier: 'free', latency: 'batch', status: 'experimental', skills: ['telemetry','behaviour-validator'], category: 'Operations', costPer1MInput: 0, costPer1MOutput: 0, costCurrency: 'USD' },
  // Free-tier fallbacks (OpenRouter)
  { id: 'researcher-free', name: 'Researcher (Free)', model: 'kimi-k2.6:free', provider: 'OpenRouter', usage: 'Deep analysis, research — free fallback', costTier: 'free', latency: '<5m', status: 'verified', skills: ['telemetry','arxiv','llm-wiki','ocr-and-documents','youtube-content'], category: 'Research', costPer1MInput: 0, costPer1MOutput: 0, costCurrency: 'USD', dailyLimit: 300 },
  { id: 'builder-free', name: 'Builder (Free)', model: 'qwen3-coder:free', provider: 'OpenRouter', usage: 'Code, config — free fallback', costTier: 'free', latency: '<2m', status: 'experimental', skills: ['test-driven-development','systematic-debugging','telemetry'], category: 'Implementation', costPer1MInput: 0, costPer1MOutput: 0, costCurrency: 'USD', dailyLimit: 200 },
  { id: 'planner-free', name: 'Planner (Free)', model: 'qwen3-coder:free', provider: 'OpenRouter', usage: 'Planning, task breakdown — free fallback', costTier: 'free', latency: '<2m', status: 'experimental', skills: ['plan','test-driven-development','telemetry'], category: 'Orchestration', costPer1MInput: 0, costPer1MOutput: 0, costCurrency: 'USD', dailyLimit: 200 },
];
...
export const bvcs: BVC[] = [
  { name: 'api-error-rate', description: 'API error rate must not exceed threshold', threshold: '< 1%', severity: 'critical' },
  { name: 'api-latency-p95', description: 'P95 latency for API calls', threshold: '< 2000ms', severity: 'warning' },
  { name: 'daily-llm-cost', description: 'Daily LLM cost', threshold: '< $50', severity: 'warning' },
  { name: 'deployment-success-rate', description: 'Deployment success rate', threshold: '> 95%', severity: 'critical' },
  { name: 'service-availability', description: 'Service availability', threshold: '> 99.5%', severity: 'critical' },
];
...
export const flowSteps = [
  {
    ...
    description: 'Buzz is the operator dialogue and approval surface. Scope is created here, approvals are given, and status is shown.',
    verified: true,
    blockers: ['Buzz-native delegation is discovery-only — by design, no dispatch (see current-operating-model.md)', 'Hermes↔Buzz return channel missing — the main gap to build', 'Buzz Builder terminal: NOT reproducible (probe 2026-08-02 llr) — should no longer be shown as stopped'],
  ...
  },
  {
    ...
    description: 'The single master registry for scope, workflow status, evidence, review and approval. No other backlog may exist.',
    verified: true,
    blockers: [],
  ...
  },
  {
    ...
    description: 'Manual dispatch or gateway dispatch establishes claim, run_id, profile and lease.',
    verified: true,
    blockers: ['General dispatcher from Ready issue does not exist yet', 'Hermes Kanban gateway limited to scratch workspaces'],
  ...
  },
  {
    ...
    description: 'Hermes runs Coordinator/Researcher profiles. Pi Builder runs bounded writes in an isolated container.',
    verified: true,
    blockers: ['Pi Builder is an experiment, not a production harness', 'Worktree mode requires cygpath -w on Windows'],
  ...
  },
  {
    ...
    description: 'Independent review of results. Codex for read-only architecture/security review. Plan Auditor for adversarial review.',
    verified: true,
    blockers: [],
  ...
  },
  {
    ...
    subtitle: 'Final Gate',
    description: 'Only a human operator may approve scope, merge, deploy and final completion. No agent approves its own work.',
    verified: true,
    blockers: [],
    outputs: ['Done status'],
  },
