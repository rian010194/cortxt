...
**Technical Story:** target-architecture.md §27 open decision #8; blocks Phase 6
(Geometric Reasoning v1)'s own exit criterion, which requires this decision
before evaluation (§23, Phase 6 Exit)

## Context

`target-architecture.md` §12.2 lists ten geometric metrics for Phase 6
...
versus which are informational. §27 #8 names this gap explicitly, and Phase 6's
own exit criterion (§23) cannot be evaluated until it is resolved: "which
metric(s) in §12.2 are decisive... is settled before this exit criterion is
evaluated."
...
1. The five metrics already consumed by `score_path`/`guidance`/
   ...
   metrics for Phase 6 v1. They may change ranking, path selection, or trigger
   `escape_attractor`.
...
### Positive
- Unblocks Phase 6's exit-criterion evaluation, which was explicitly gated on
  this decision.
...
1. **Treat all ten metrics as decisive, add the missing five to `score_path`
   ...
   evaluation evidence behind them (violates §12.4's own "weights and
   thresholds are policy data and must be evaluated against fixtures, not
   hidden in prompt text"), and changes live scoring behavior as a side effect of
...
3. **Leave §27 #8 unresolved and evaluate Phase 6's exit criterion anyway.**
...
## Validation
- [ ] Phase 6's exit-criterion evaluation, when it runs, states explicitly
...
- Trigger: Phase 6's exit-criterion evaluation actually runs and finds one of
