...
| 047 | Surface operation-contract parity and the control-plane / coordinator-agent boundary | **Proposed** | CLI, MCP and OS consume one shared operation contract with identical mandate/approval checks (parity = same contract and controls, not feature-simultaneity); MCP follows the same applicable controls incl. already-granted autonomy and adds no new per-call confirmation; the deterministic control plane owns authority, a coordinator agent is optional and never per-runtime, and native runtime delegation must be explicitly qualified. References ADR-018/024/032/038/039/040/041/042/044; does not gate ADR-045/#495 or the v1/v2 digest question. |
