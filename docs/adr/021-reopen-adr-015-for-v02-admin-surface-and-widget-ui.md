...
**Technical Story:** GitHub issue rian010194/cortxt#157; v.02-milestone wayfinder Phase 0 (`.hermes/plans/2026-08-18-v02-milestone-wayfinder.md`)

## Context

ADR-015 (Cortxt F1 — First Wedge and Product Surface) pausade web/Operator Cockpit as a product surface in favor of "repository-native + CLI (primarily)" and left an explicit review trigger: "observed user demand points to another wedge" (ADR-015, Expiry/Review Trigger section — ADRs in this repo are not §-numbered internally, unlike target-architecture.md).
...
This ADR exists solely to carry that review trigger to a formal decision. It does not authorize any implementation. Implementation authorization is gated separately: Phase 2+ of the v.02 wayfinder does not count as authoritative product surface until ADR-021 is Status: Accepted, set by Rikard.
...
- This does not decide naming, the credential-storage security model, pricing, or the addon review process — those are separate open questions in §6 of the v.02 vision doc and remain open. They are addressed in later wayfinder phases (Phase 1 security model, Phase 6 pricing), not by this ADR.
- This does not authorize starting Phase 2+ of the wayfinder. Only Rikard setting Status: Accepted on this ADR does that, per the wayfinder's own standing decision ("Phase 0 (ADR) always runs first. Nothing in Phase 2+ counts as authoritative product surface until ADR-021 is Accepted").

This ADR is therefore **necessary but not sufficient** for Phase 2+ implementation. It clears the ADR-level gate; the operator's Accepted decision unlocks the build gate.
...
- The wayfinder's Phase 0 gate becomes explicit and auditable: no Phase 2+ work is authoritative before ADR-021 Accepted. This protects against the common failure mode where prototypes silently become de facto product decisions.
...
3. **Defer the surface question to a later ADR after Phase 2 is already built.** — Rejected: that inverts the wayfinder's own standing decision (Phase 0 ADR first) and would make the ADR a post-hoc rationalization rather than a gate. The operator's instruction is explicit that the ADR precedes the build, not follows it.
...
- [x] ADR does not itself authorize starting Phase 2+ of the wayfinder — only Rikard setting Status: Accepted does that.
