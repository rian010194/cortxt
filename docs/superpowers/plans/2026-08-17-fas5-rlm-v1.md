    ...
    def _spawn_rlm_node(self, parent_session_id, task_id, context_ref, config, depth,
        ...
        try:
            with os.fdopen(fd, "w", encoding="utf-8") as handle:
                ...
                # the default set (corrected during review).
...
- Produces: `gate.DataClassGate` (new class, `agent-platform/runtime/tools/gate.py`
  ...
  passing unmodified); `Coordinator.run_node` gains an
  `allowed_data_classes: frozenset[str]` parameter (default
  `frozenset({"L0", "internal"})`) and threads it into the JSON config payload
  written by `_spawn_rlm_node` so children inherit the parent's policy instead
  of re-hardcoding the set; `Coordinator.run_node`'s return dict gains
...
# Inherit the parent's allowlist from the JSON payload; the same value is
# passed back into Coordinator.run_node below so grandchildren keep the policy.
allowed_data_classes = frozenset(payload.get("allowed_data_classes", ["L0", "internal"]))
...
`output_size` keys to sum — `_index_to_result` now reads these from the
child's own `result.available` event payload via `child_tree.session_doc["events"]`,
and `rlm_child_cli.main()`'s leaf branch writes them into that event:
`cost` uses the same placeholder unit-cost formula as Task 13/17
(`0.01` per leaf invocation) until `TextInferencePort`'s usage-reporting field
is confirmed, and `output_size` is `len(str(value))` matching `rlm_engine.py`'s
existing `RLMRun.output_length` convention.)
...
- **Type consistency checked:** `ContextReference` (Task 1) is the same type
  ...
  `run_eval_class`).
- **Data-class allowlist fix (review pass 3):** `Coordinator.run_node` now
  takes `allowed_data_classes: frozenset[str]` (default `frozenset({"L0",
  "internal"})`); `_spawn_rlm_node` writes it as a sibling key in the JSON
  config payload, and `rlm_child_cli.main()` reads it back and forwards it
  into the child's own `Coordinator.run_node` call. This removes the
  hard-coded `{"L0", "internal"}` literals at both the Coordinator and child
  CLI call sites and lets a future phase override the allowlist without
  touching core mechanics. A new test in Task 6 proves a non-default value
  survives the JSON round-trip to spawned grandchildren.
- **Fixture materialization fix (review pass 3):** `CodingFixture` and
  `ResearchFixture` no longer expose a `context_ref` attribute (the original
  locators were unreadable: "." and "doc-set"). Task 13 and Task 17 now call
  `materialize(fixture, dest_dir)` to write the fixture to real files on disk
  and pass the returned `ContextReference` into `Coordinator.run_node`.
- **Result-metrics fix (review pass 3):** `rlm_child_cli.main()`'s leaf
  `result.available` event now includes `cost` and `output_size`, and
  `_index_to_result` reads those fields from `NodeDocs.session_doc["events"]`
  so Task 9's post-hoc bounds summation has real values to aggregate.
  `cost` uses the same placeholder unit-cost formula as Task 13/17
  (`0.01` per leaf invocation) until `TextInferencePort`'s usage-reporting
  field is confirmed; `output_size` is `len(str(value))`, matching
  `rlm_engine.py`'s `RLMRun.output_length` convention.
