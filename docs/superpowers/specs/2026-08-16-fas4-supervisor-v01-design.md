...
> **Dependency: Fas 2 and Fas 3 are merged.** Everything below extends
> `agent-platform/runtime/` — `session_state.py` (append-only, hash-chained,
> atomically-written per-session event log with optimistic-concurrency `append()`),
> `agent_loop.py` (single-threaded synchronous `AgentLoop.run()`), and Fas 3's coding
> profile (read/search/patch/test/diff tools, copy-in workspace, blocking subprocess
> execution sandbox). No `supervisor/` package exists yet — this is new code, not a
...
- **M1:** two *independent* Fas-3-coding-profile child runs, spawned together,
  ...
  handoff.
- **M2:** child 2 depends on child 1's *output* — child 1 computes a diff (via
  Fas 3's existing `diff_workspace`), Supervisor copies that diff into child 2's
  fresh copy-in workspace (`apply_incoming_patch`, new), child 2 applies it via
  Fas 3's existing `apply_patch` before running. Proves dependency joins and
  workspace-artifact handoff, which M1 alone does not exercise.
...
adopted. Full review transcript: `hermes sessions export --session-id
20260816_175447_294d2d`.
...
| `coding_loop_cli.py` | New CLI entry point for a child process (`--session-id`, `--store`, `--config-json`). Starts the heartbeat daemon thread and runs Fas 3's `AgentLoop`/coding profile, both through the same `SessionWriter`. The child inherits no Python object graph from Supervisor — only filesystem references. |
...
`join.waiting`, `join.satisfied`, `session.reattached`.

**`tools/patch.py` extension (M2):** `apply_incoming_patch` — Supervisor copies
child 1's diff artifact into child 2's fresh copy-in workspace before child 2
starts, applied via Fas 3's existing `apply_patch`.
...
Root: ADMITTED → FRAMING (splits into a dependent subtask pair) → READY_TO_REASON
  → EXECUTING
    ...
    IF child 1 SUCCEEDED:
      diff artifact extracted (tools/patch.py diff_workspace, already computed
        at child 1 terminal)
      budget.reclaimed (child 1's unused) → budget.transferred to child 2
      apply_incoming_patch against child 2's fresh copy-in workspace
...
| `apply_incoming_patch` fails (M2, patch doesn't apply cleanly) | Child 2 never spawned; root → `blocked`, reason "patch handoff failed", pointing at child 1's diff artifact for operator diagnosis. |
...
- Full workspace snapshotting for non-text artifacts — M2 uses patch-based handoff
  only; a full-snapshot fallback for non-text-patchable fixtures is named as a
