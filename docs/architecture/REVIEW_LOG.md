...
| 2026-09-17 | ADR-051 | Added and accepted by operator (OD-1) - local dialogue routes on the action host: a second, closed route class extending ADR-038 (six token-gated `/api/dialogue/*` routes, effects only from explicit operator POSTs, permission requests always denied, no code path to action ports/GitHub/dispatch/Runs); committed as rev2 (draft sha16 85af556a8f18174b, audited PASS by four non-author gates) with only the Status and Deciders lines changed; D9 remains conditional on DEC-2 | Operator approval |
