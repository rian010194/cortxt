...
def test_invoke_delegates_to_injected_invoke_hermes_unchanged():
    ...
    def fake_invoke_hermes(profile, prompt, *, timeout_seconds, model=None, provider=None, cwd=None):
        calls.append((profile, prompt, timeout_seconds, model, provider, cwd))
    ...
    assert calls == [("researcher", "do research", 300, "m", "p", None)]


def test_invoke_propagates_hermes_invocation_error_unwrapped():
    def raising_invoke_hermes(profile, prompt, *, timeout_seconds, model=None, provider=None, cwd=None):
...
def test_default_constructor_delegates_to_live_hermes_invoker_module_lookup():
    ...
    fake.assert_called_once_with("builder", "do it", timeout_seconds=60, model=None, provider=None, cwd=None)
...
def test_explicit_invoke_hermes_still_takes_priority_over_live_lookup():
    ...
    def explicit_fn(profile, prompt, *, timeout_seconds, model=None, provider=None, cwd=None):
        calls.append((profile, prompt, timeout_seconds, model, provider, cwd))
        return {"status": "succeeded", "profile": profile}
    ...
    assert calls == [("builder", "do it", 60, None, None, None)]
